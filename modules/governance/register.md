...
The following is a risk assessment for the "Loan Warehousing System" of a tier one financial services organisation.
This assessment will consider financial, legal \& regulatory, operational and reputational risk aspects.
\blank[line]
Consulting the respective CIO stakeholders, the following business critical assets have been identified and extracted from the organisations inventory management system within the scope of this risk assessment:
...
Before a Risk Assessment can be conducted, the organisations posture towards the risk appetite has to be defined and agreed upon by all of the stakeholders. For that it has been proposed to set the following acceptance ratings for this financial services organisation, which takes the suggestions from the \infull{ISF} (\ISF\) towards defining a risk appetite into account: \blank[line]
...
Then, a formal risk evaluation has to be conducted, of which the methodology is described and shown in much detail in section 2 on page \at[riskeval]. The outcome of these risk evaluations, each one for a financial, operational and regulatory risk (see table \in[table:birt] \about[table:birt] and table \in[table:isa] \about[table:isa]), an overall risk impact together with an assessment for the likelihood (see table \in[table:likelihood] \about[table:likelihood]) of the risk occurring for each asset, has been incorporated into the risk register.
...
The following table shows the risk register for each and every asset identified in this information management system as seen in the separatly provided Information Asset Inventory document: \blank[line]
...
\startxtable[option=max,split=yes,header=repeat]
    ...
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 9 \stopxcell \startxcell Underwriter \stopxcell \stopxcellgroup \startxcell {\bf Deficient risk assessment skills} aligned with organisation risk appetite for determining solvency. \stopxcell \startxcellgroup[align={middle,lohi}] \startxcell[background=color,backgroundcolor=lightgray] Longer Term \stopxcell \startxcell 4 \stopxcell \startxcell 3 \stopxcell \startxcell 1 \stopxcell \startxcell {\bf 3} \stopxcell \startxcell {\bf 2} \stopxcell \startxcell[background=color,backgroundcolor=orange] {\bf 6} \stopxcell \startxcell No \stopxcell \stopxcellgroup \startxcell Up-to-date organisation strategies, less aggressive risk appetite and well trained staff. \stopxcell \stopxrow
...
As seen, the risk ratings in this risk assessment identifies risk levels that requires containment, management and attention to reduce it. As a result, a set of mitigation action plans to reduce the reported risks have been highlighted in a separate section on page \at[riskeval] that lays out the proposed treatment plan. \blank[line]
...
With input form the IRAM2 framework of the \infull{ISF} (\ISF) the following table defines the different risk levels: \blank[line]
...
Derived from this the following risk framework matrix defines the impact by score: \blank[line]
...
* Accepting the risk (risk appetite)
...
\startxtable[option=max,split=yes,header=repeat]
    ...
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 2 \stopxcell \startxcell System Inaccessibility \stopxcell \stopxcellgroup \startxcell Reduce dependency of service provider and each data locally \stopxcell \startxcell 1. Multiple service provider \\ 2. Short-term software based solution \stopxcell \startxcellgroup[align={middle}] \startxcell Medium \stopxcell \startxcell Low \stopxcell \startxcell Transfer \stopxcell \stopxcellgroup \startxcell Vendor Management Division \stopxcell \startxcell[align={middle}] Q2 2016 \stopxcell \startxcell Monitor availability and service levels, compare peer solutions. \stopxcell \stopxrow
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 3 \stopxcell \startxcell Third-Party System Glitch \stopxcell \stopxcellgroup \startxcell Leverage complementary solutions or buying insurance \stopxcell \startxcell 1. Available alternatives \\ 2. Insurance coverage \stopxcell \startxcellgroup[align={middle}] \startxcell High \stopxcell \startxcell Medium \stopxcell \startxcell Transfer \stopxcell \stopxcellgroup \startxcell Vendor Management or Legal Division \stopxcell \startxcell[align={middle}] Q3 2016 \stopxcell \startxcell Monitor up-time and historical data, check insurance rates. \stopxcell \stopxrow
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 4 \stopxcell \startxcell Data Loss \stopxcell \stopxcellgroup \startxcell Building redundancy systems and good forensics team or outsource \stopxcell \startxcell 1. Redundancy by design \\ 2. Forensics team and kit \\ 3. Outsource \stopxcell \startxcellgroup[align={middle}] \startxcell Medium \stopxcell \startxcell Low \stopxcell \startxcell Mitigate or Transfer \stopxcell \stopxcellgroup \startxcell Storage Division \stopxcell \startxcell[align={middle}] Q4 2015 \stopxcell \startxcell Monitor disk failures and leverage archiving systems or service levels. \stopxcell \stopxrow
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 5 \stopxcell \startxcell Broken Devices \stopxcell \stopxcellgroup \startxcell Spare hardware availability and quick root-cause analysis \stopxcell \startxcell 1. Provide redundant hardware \\ 2. Effective hardware team \stopxcell \startxcellgroup[align={middle}] \startxcell Low \stopxcell \startxcell Low \stopxcell \startxcell Accept \stopxcell \stopxcellgroup \startxcell Hardware Division \stopxcell \startxcell[align={middle}] Ongoing \stopxcell \startxcell Monitor technical faults and frequency of hardware failure. \stopxcell \stopxrow
    ...
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 11 \stopxcell \startxcell Human Resources \stopxcell \stopxcellgroup \startxcell Appropriate budget for staff \\ Outsourcing strategies for critical personnel \stopxcell \startxcell 1. Contingency budget in place for emergencies \\ 2. Contract with outsourcing organisation \stopxcell \startxcellgroup[align={middle}] \startxcell Low \stopxcell \startxcell Low \stopxcell \startxcell Accept \stopxcell \stopxcellgroup \startxcell HR Division \stopxcell \startxcell[align={middle}] Ongoing \stopxcell \startxcell Monitor retention rate and register movers, joiners and leavers list. \stopxcell \stopxrow
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 12 \stopxcell \startxcell Regulatory Incompliance \stopxcell \stopxcellgroup \startxcell Leverage Internal Audit to prevent incompliance \stopxcell \startxcell 1. Frequent internal auditing \\ 2. Implement audit findings in time \stopxcell \startxcellgroup[align={middle}] \startxcell High \stopxcell \startxcell Medium \stopxcell \startxcell Mitigate \stopxcell \stopxcellgroup \startxcell Internal Audit Division \stopxcell \startxcell[align={middle}] Ongoing \stopxcell \startxcell Central monitoring for audit findings. \stopxcell \stopxrow
    ...
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 15 \stopxcell \startxcell Data Leakage \stopxcell \stopxcellgroup \startxcell Implementation of Data Leakage Prevention (DLP) systems. \\ Reduce access to need-to-know basis. \stopxcell \startxcell 1. Putting a DLP system in place \\ 2. Re-define ACL list for privileged users \stopxcell \startxcellgroup[align={middle}] \startxcell Medium \stopxcell \startxcell Low \stopxcell \startxcell Mitigate \stopxcell \stopxcellgroup \startxcell CISO Division \stopxcell \startxcell[align={middle}] Q4 2016 \stopxcell \startxcell Perimeter scanning for egress connections and content filtering, classification systems. \stopxcell \stopxrow
    \startxrow \startxcellgroup[align={middle,lohi}] \startxcell 16 \stopxcell \startxcell Natural Disaster \stopxcell \stopxcellgroup \startxcell Redundant facilities and physical protection or insurance against natural disaster. \stopxcell \startxcell 1. Multiple DR sites \\ 2. Hardening Physical structure of buildings \\ 3. Buying insurance \stopxcell \startxcellgroup[align={middle}] \startxcell Low \stopxcell \startxcell Low \stopxcell \startxcell Avoid, Accept or Transfer \stopxcell \stopxcellgroup \startxcell Facilities Division \stopxcell \startxcell[align={middle}] Ongoing \stopxcell \startxcell Monitor regional data for probability of natural disasters. \stopxcell \stopxrow
    \stopxtablebody
\stopxtable}
\stopalignment
...
The final matrix as shown below is the risk level of impact scale that was used in the risk assessment and risk evaluation sections. This is used as the official risk framework for this tier one financial services organisation. \crlf
