...
Building a logical flow, the sections will cover the validity of the case study of connected cars followed by the review of existing models on data analysis and solutions utilised in the field. Continuing with the elaboration on the data capturing techniques for \infull{CPS}, behavioural data will be constructed through different takes on system profiling models with references to state-of-the-art research. This will eventually lead to answer the question on how to deal with anomaly detection. In order to relate to current cyber threats and to give evidence of the contribution this project provides, the closing critique will be dealing with the threat prediction methods. \par
...
Having had the sub-set of \infull{CPS} defined for this project, the type of profiling and behaviour on navigation explained, the very last aspect on why all of this is relevant lies in the potential risk of cyber threats. Autonomous vehicles will be exposed to threats from the internet \cite[authoryear][6803166] and are facing real problems of cyber threats. This brings a whole new set of challenging situations \cite[authoryear][adouane2016autonomous], like the particular one of cyber-hijacking that has to be dealt with. \par
...
% Total number of words per section: 408/400

\startsection[title={Existing Models on Data Analysis},reference=section:existingmodels]
% Relevancy
This section will examine the existing models that are available and used in the industry that can help analysing the motion patterns of connected cars. Most of the work has been done on network traffic log analysis and this will be looked at, more specifically, how to adapt the interpretation of sensor readings into the algorithm. \par
...
Most of the historical work was conducted on \infull{IDPS}s and the analysis on network log traffic \cite[authoryear][Krugel:2002:SSA:508791.508835]. Although it is certainly a knowledge-base to be built from, the fast changing constellation of connected systems is in need of a new approach on dynamic analysis. \par
...
By looking specifically on how profiling and behavioural analysis has been analysed on different types of traffic accident scenarios \cite[authoryear][Parker1995571], a good profiling pattern can be derived from the data being processed. \par
% Work done
...
% Total number of words per section: 293/400
...
For the definition of the data-set and capturing techniques, this paper is looking at established methods already in use within the aviation industry. The global tracking system \infull{ADS-B} (ADS-B) defines prerequisite data types that need to be sent through radar capturing facilities \cite[authoryear][McCallie201178]. Among the required fields are flight ID, geolocation, time, speed and various other sensor readings. For an illustration of the raw \infull{CSV} (CSV) data, see \in{Table}[table:ads] in the Appendix. The following screenshot of a live ADS-B tracking along an airport gives an indication how this is going to be used in relation to connected cars for this project: \par
...
\stopplacefigure
More important, there is already some form of data capturing available with electric car-sharing providers, the so-called Car-Sharing Modules (CSM) \cite[authoryear][lee2011electric]. Although proprietary, the captured data includes the identification (ID) of the individual renting the car, readings of battery charge, location and speed and log data from the machine learning application. \par
...
% Total number of words per section: 322/400
...
\startxcell Statistical anomaly detection technique that calculates an anomaly score for each packet that it sees; it forwards the packets to a correlation engine for intrusion detection purposes when a predefined threshold was crossed \stopxcell
\startxcell Network based statistical anomaly detection \stopxcell
\stopxrow
...
This new approach of real-time specification based correlation \cite[authoryear][Sekar:2002:SAD:586110.586146] will give a boost to the "time to detect" factor on malicious behaviour. \par
Another aspect regarding connected cars are ad-hoc networks and the intrusion detection of such networks. It has been shown that anomaly detection methods are particularly advantageous in such circumstances \cite[authoryear][Zhang:2000:IDW:345910.345958]. \par
% Work done
The novel approach in cyber threat detection on connected cars in this paper has been advanced through getting rid of signature based detection mechanisms and replacing it with anomaly detection algorithms that are more purposeful in the connected wireless transportation landscape. \par
% Results found
...
% Total number of words per section: 267/400
...
Integrating different prediction methods into a framework for security \cite[authoryear][Chien2012] allows applying a proactive approach with the specific use case of remote cyber hacking, demonstrated by the action of hijacking. \par
...
% Work done
This paper has applied the sequential Bayesian filtering methods on the specific simulations with connected cars and has compared the results in contrast to the machine learning models.
% Results found
It turns out that with the big data created through the sensors whilst in transit it is by one order of magnitude more precise in creating a baseline to compare against and hence these methods are more appropriate for cyber threat prediction. \par
...
% Total number of words per section: 297/400

\blank[line]

This chapter has considered the available literature in depth in regards to the paper's research question on proactive threat detection methods with the use case of connected cars and has critically evaluated the content. In the next chapter the design of the project, including the experiments and the case study can be found. \par
...
% Total number of words: 2091/3000
