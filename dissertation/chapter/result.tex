...
The baseline for comparison is primarily the average function of genuine routes marked as known-good and the correlation to this line with the added correction of errors. It has been shown that the behavioural prediction creates a function that is better aligned to this baseline as well as reducing the error correction to a smaller deviation threshold. \par
Further, the Bayes filter applications have been compared with each other as well. Depending on the scenario the sequential Bayesian filtering produced the best outcomes. The comparisons are attached to the Appendix section at the end of this paper. \par
It has been shown that overall with the same inputs of selected routes and the application of different methods for data analysis, the results on detecting anomaly behaviour differ with their outcome quite extensively. \par
The examples are shown here: \par
\stopsection

% Total number of words per section: 163/350
...
Metrics were chosen to look at the effectiveness on improving the detection algorithms via a time dimension and precision. These metrics do not depend on the data types the examples chosen. \par
Validating the results was done through showing that improvement was done through metrics of time-frame and precision. \par
...
% Total number of words per section: 141/350
...
To put this paper into perspective in terms of the contribution to the community, it will compare the presented novel idea of using estimation techniques for anomaly detection against the methods described in the publications above. The outcome of this comparison is an increased window to react to cyber threats and a precision of 5\% deviation from the norm which has been shown. \par
...
% Total number of words per section: 267/350
