...
\startplacefigure[title={Pairwise correlation},list={Correlation},reference=figure:correlation]
...
\startplacefigure[title={ADS-B CSV database},list={ADS-B CSV},reference=figure:ads_csv]
...
Recursive Bayesian estimation or Bayes filter; general probabilistic approach for estimating an unknown probability density function recursively over time using incoming measurements and a mathematical process model \par
Applications (Kalman filter, Markov process, Hidden Markov model (HMM)) \par
Sequential Bayesian filtering (filtering, smoothing, prediction) \par
extensively used in control and robotics \par
references in bib \par
