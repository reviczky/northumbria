...
\starttext
% \colored[r=0.33,g=0.61,b=0.71]{Northumbria}
% \definecolor[northumbria][h=549CB5]
\definecolor[northumbria][c=.54,m=.14,y=0,k=.29]
% \startcolor[northumbria]{Northumbria}\stopcolor

\setupheadertexts[\midaligned{\reuseMPgraphic{logo}{fgcolor=black,bgcolor=northumbria,width=0.5mm}}]
% \midaligned{\reuseMPgraphic{logo}{fgcolor=black,bgcolor=northumbria,width=0.5mm}}
\setupfootertexts[\setups{footer}]

% \framed{\midaligned{\studentname \ (\studentnumber)}}
\startsetups[footer]
\framed[width=100mm,align=middle,frame=off]{\midaligned{\ss \studentname \\ (\studentnumber)}}
\stopsetups

\setuphead[subject][alternative=middle,textstyle={\ssb}]
\startsubject[title={Viva voce}]
% You should expect to spend about 10 minutes explaining the work that you have done and the key findings and about 10 minutes demonstrating any software product you have created. You should then answer any questions that your supervisor and second marker have. These questions may cover:
% Technical details on the construction and function of the product
% The research process
% The conclusions from the research
% The aim and context of the research
% Your evaluation of the research

% At the viva, you are normally allowed about 20 minutes to give a broad outline of your work and to demonstrate your product. On rare occasions, the product may be a detailed design or other non-software product. If this is the case, you should do a walk through the product. Both your supervisor and second marker will then ask questions to determine the level of your understanding. Typically, each marker will ask three questions, although follow-up question may be needed to extract the information needed to assess you. These questions should include at least one question on the following issues:
% The function and structure of the product, either in whole or covering a specific detail.
% The research process.
% The context of the research and/or the potential value of the conclusions.
% Unless the markers need to address concerns about possible misconduct, the viva will normally last no more than 30 minutes. After the viva, the markers should collaboratively establish a mark for the following criteria:

\define\studentname{Adam Reviczky}
\define\studentnumber{14044980}

\startsubsubject[title={MSc Title}]
{\bf%
\startquotation[middle]
Proactive threat detection of cyber-physical systems using Bayesian estimation: connected cars as a case study
\stopquotation
}
\stopsubsubject

\startsubsubject[title={Introduction}]
% 9-10: An outstanding presentation was delivered, which explained the nature of the research and provided valuable and thoughtful insight into innovative research.

\startitemize[joinedup,nowhite]
\sym{»} Example of Malaysian Airline vanishing (radar, deviation from course)
\sym{»} Connected Cars relevance: Hijacking kids from school ride
\stopitemize
\stopsubsubject

\startsubsubject[title=\Words{Table of contents}]
High-level outline of the dissertation: \par
\blank[line]
\startitemize[joinedup,nowhite]
\sym{»} Abstract
\sym{»} 1: Introduction
 \startitemize
  ...
  \item Aim / Objectives (define connected cars; create scenarios; compare density functions; algorithms for profiling&behaviour; validation of signatures; demonstrating the improvements; suggest fail-safe actions; propose warning system)
  \item Key Findings and Contribution
 \stopitemize
\sym{»} 2: Methodology
 \startitemize
  \item Research Question: \startquote How can cyber resilience be improved proactively to predict cyber threats in real time on connected cars in such a way as to counter cyber-attacks? \stopquote
  \item Research Design: quantitative data research (numerical data collection, CSV)
  \item Data Analysis: Stan (R based) and PyMC (Python based)
  \item Experiments 1 & 2
 \stopitemize
\sym{»} 3: Critical Review of the Literature
 \startitemize
  \item Usability Study / Existing Models on Data Analysis: current machine learning solutions
  \item Data Capturing: ADS-B, aviation industry and CSM (car-sharing module)
  \item Behavioural Profiling: Profile grouping and Behavioural Analysis (mostly network logs)
  \item Anomaly Detection: AI (machine learning), signatures, estimation techniques - deviation from norm
  \item Threat Prediction: definition of cyber threat, attack vectors (hijacking, damage, espionage, terror)
 \stopitemize
\sym{»} 4: Design \\
 Sequential Bayesian Filtering
 \startitemize
  \item Experimental Work: RStan simulation on the experiments
  \item Data Collection: speed, geolocation, elevation, orientation, date-stamp, ID
  \item Estimation Model: Bayes filters (filtering, smoothing, prediction)
  \item Correlation Graphs: mean and deviation functions (comparison)
  \item Case Study and Particular Events: connected cars with sensors on a hijacking scenario
  \item Similar Requirements for the Community: Hooking in to the "Automotive Cyber Security", "Automotive Grade Linux", "Automotive Cyber Security", "Alliance of Automobile Manufacturers"
 \stopitemize
\sym{»} 5: Results Collected and Analysis
 \startitemize
  \item Techniques Used: Analytical (data analysis); reasoned evaluation
  \item Comparison of the Results: 5\% threshold
  \item Reflection on the Work Done: sensors, prediction, examples
  \item (Results) Evaluation: Improvements on "time to detect" and precision "deviation threshold"
  \item Contribution to the Community: main publication \startquote Potential Cyberattacks on Automated Vehicles \stopquote
 \stopitemize
\sym{»} 6: Discussion
 \startitemize
  \item Project Evaluation: Objectives met; techniques used & missed
  \item Correlation Effectiveness: false-positives
  \item Ethical Considerations: automated actions from results (collisions)
 \stopitemize
\sym{»} 7: Conclusion & Suggestions for Further Study
 \startitemize
  \item Addressing the Problem Statement: proactive approach; built-in security
  \item Review of Objectives: details
  \item Reflection on Work Done: extensive work on applying estimation techniques
  \item Mapping of the Results: in regards to the objectives
  \item Further Work: warning system; tracking system; re-active solutions; ITS (Intelligent Transportation Systems)
 \stopitemize
\sym{»} Appendix
 \startitemize
  \item Timetable/Deadlines; Project Plan; Supervisor Meetings
  \item detailed Figures/Diagrams and Charts; Tables (CSV, database); Source Code(s)
  \item Project Log; Risk Assessment & Ethical Form
\stopitemize
\stopitemize
\stopsubsubject

\startsubsubject[title={Closing}]
Demonstration of a novel, innovative approach on cyber resilience on examples of autonomous connected cars. \par
\stopsubsubject

\startsubsubject[title={Questions & Answers}]
% 9-10: Your answers demonstrated a deep and insightful understanding of research processes and you were able to discuss original research findings.

Question {\switchtobodyfont[xits]①}: \startquote \it Technical details on the construction and function of the product \stopquote \par
\startitemize[joinedup,nowhite]
\sym{»} open source Bayesian libraries
\stopitemize
Question {\switchtobodyfont[xits]②}: \startquote \it The research process \stopquote \par
\startitemize[joinedup,nowhite]
\sym{»} justification and applicability of the problem; reading literature; looking at existing solutions; coming up with a solution
\stopitemize
Question {\switchtobodyfont[xits]③}: \startquote \it The conclusions from the research \stopquote \par
\startitemize[joinedup,nowhite]
\sym{»} improvements achieved through predictions; new approach on cyber threats
\stopitemize
Question {\switchtobodyfont[xits]④}: \startquote \it The aim and context of the research \stopquote \par
\startitemize[joinedup,nowhite]
\sym{»} draft publication in work for peer-review on behaviour analysis
\stopitemize
Question {\switchtobodyfont[xits]⑤}: \startquote \it Your evaluation of the research \stopquote \par
\startitemize[joinedup,nowhite]
\sym{»} insightful; deep-dive; expanded my horizon in terms of tools and techniques on research
\stopitemize
\stopsubsubject
